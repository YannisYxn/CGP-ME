...
for individual_index in range(individual_size):
    ...
    for train_index, test_index in skf.split(X=data[:,:-1], y=data[:,-1]):
        ...
        base_estimator.fit(np.hstack((train_X[:,:28*28*3], mapped_train_X_0)), train_y)
        predict_y_list.append(base_estimator.predict(np.hstack((test_X[:,:28*28*3], mapped_test_X_0))).tolist())
        base_estimator.fit(np.hstack((train_X[:,:28*28*3], mapped_train_X_1)), train_y)
        predict_y_list.append(base_estimator.predict(np.hstack((test_X[:,:28*28*3], mapped_test_X_1))).tolist())
        base_estimator.fit(np.hstack((train_X[:,:28*28*3], mapped_train_X_2)), train_y)
        predict_y_list.append(base_estimator.predict(np.hstack((test_X[:,:28*28*3], mapped_test_X_2))).tolist())
        base_estimator.fit(np.hstack((train_X[:,:28*28*3], mapped_train_X_3)), train_y)
        predict_y_list.append(base_estimator.predict(np.hstack((test_X[:,:28*28*3], mapped_test_X_3))).tolist())
